...
def prepare_inception_metrics(dataset, parallel, no_is=False, no_fid=False):
  ...
  if dataset == 'CIFAR10':
      dataset_name = 'C10' 
  data_mu = np.load('./inception_pytorch/'+dataset_name+'_inception_moments.npz')['mu']
  data_sigma = np.load('./inception_pytorch/'+dataset_name+'_inception_moments.npz')['sigma']
  # Load network
